...
- Descripción del programa: Este programa recibe por pantalla el número de términos e imprime de la serie Triangular hasta ese número sin superarlo
    ...
    Salvedad: El programa solo funciona si se le ingresan números enteros positivos y no mayores a 92600, ya que si no podría haber un desbordamiento de datos por culpa del que el número superaría la capacidad del unsigned long int. De lo contrario no se garantizan resultados 
...
// Inicio función PrintTriangularSeries. Esta función va a imprimir la serie Triangular hasta el número de términos indicado y no devuelve nada
void PrintTriangularSeries(unsigned long int n1, int term, int limit) {
    // n1: Primer dígito y dígito imprimible de la serie Triangular
    ...
    // Imprimimos la serie Triangular siempre y cuando el número del término actual sea menor al límite ingresado
    if ( term <= limit ) { // Condición cota de la función para el llamado recursivo
        ...
        if ( term == limit ) { // Si está en el último dígito imprime punto "."
...
int main() { // Inicio función principal
    ...
    printf("\nIngrese el n%cmero de t%crminos que desea ver de la serie Triangular: ", 163, 130);
