...
- Descripción del programa: Este programa recibe por pantalla el número de términos e imprime de la serie de Padovan hasta ese número sin superarlo
    ...
    Salvedad: El programa solo funciona si se le ingresan números enteros positivos y no mayores a 86, ya que si no podría haber un desbordamiento de datos por culpa del que el número superaría la capacidad del unsigned long int. De lo contrario no se garantizan resultados 
...
// Inicio función PrintPadovanSeries. Esta función va a imprimir la serie de Padovan hasta el número de términos indicado y no devuelve nada
void PrintPadovanSeries(unsigned long int n1, unsigned long int n2, unsigned long int n3, int limit) {
    // n1: Primer dígito y dígito imprimible de la serie de Padovan
    ...
    // Imprimimos la serie de Padovan siempre y cuando el límite no llegue a 0
...
int main() { // Inicio función principal
    ...
    printf("\n");
    // Llamado a la función que va a imprimir la serie con los tres primeros dígitos de esta (1, 0, 0) y el límite ingresado
