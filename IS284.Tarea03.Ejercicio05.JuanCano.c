...
- Descripción del programa: Este programa recibe por pantalla el número de términos e imprime de la serie de Perrin hasta ese número sin superarlo
    ...
    Salvedad: El programa solo funciona si se le ingresan números enteros positivos y no mayores a 79, ya que si no podría haber un desbordamiento de datos por culpa del que el número superaria la capacidad del unsigned long int. De lo contrario no se garantizan resultados 
...
// Inicio función PrintPerrinSeries. Esta función va a imprimir la serie de Perrin hasta el número de terminos indicado y no devuelve nada
void PrintPerrinSeries(unsigned long int n1, unsigned long int n2, unsigned long int n3, int limit) {
    // n1: Primer digito y digito imprimible de la serie de Perrin
    ...
    // Imprimimos la serie de Perrin siempre y cuando el limite no llegue a 0
...
int main() { // Inicio función principal
    ...
    // Llamado a la función que va a imprimir la serie con los tres primeros digitos de esta (3, 0, 2) y el límite ingresado
