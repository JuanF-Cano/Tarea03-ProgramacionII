...
unsigned long int GetUpperHalfFactorial(int n, int limit) {
    if (n > limit) { // Inicio condición cota
        return n * GetUpperHalfFactorial(n - 1, limit); // Llamada recursiva a la función
...
// Inicio función PrintCatalanSeries. Esta función va a imprimir la serie de Catalán hasta el número de términos indicado y no devuelve nada
void PrintCatalanSeries(unsigned long int n1, int term, int limit) {
    // n1: Primer dígito y dígito imprimible de la serie de Catalán
    ...
    if ( term <= limit ) { // Condición cota de la función para el llamado recursivo
        ...
        if ( term < limit ) { // Si no está en el último dígito imprime coma ","
