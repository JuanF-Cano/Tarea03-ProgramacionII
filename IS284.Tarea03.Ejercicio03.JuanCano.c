...
- Descripción del programa: Este programa recibe por pantalla el número de términos e imprime la serie de Lucas hasta ese número sin superarlo
    ...
    Salvedad: El programa solo funciona si se le ingresan números enteros positivos y no mayores a 47, ya que si no podría haber un desbordamiento de datos por culpa del que el número superaria la capacidad del unsigned long int. De lo contrario no se garantizan resultados 
...
// Inicio función PrintLucasSeries. Esta función va a imprimir la serie de Lucas hasta el número de terminos indicado y no devuelve nada
void PrintLucasSeries(unsigned long int n1, unsigned long int n2, int limit) {
    // n1: Primer digito y digito imprimible de la serie de Lucas
    ...
    // Imprimimos la serie de Lucas siempre y cuando el limite no llegue a 0
    if ( limit > 0 ) { // Condición cota de la función para el llamado recursivo
        ...
        // Pasa n2 como n1, n1 + n2 como n2 y el limite se reduce en 1 por llamado para ir descontanto cada digito impreso
...
int main() { // Inicio función principal
    ...
    // Llamado a la función que va a imprimir la serie con los dos primeros digitos de esta (2 y 1) y el límite ingresado
