...
- Descripción del programa: Este programa recibe por pantalla el número de términos e imprime de la serie de Narayana hasta ese número sin superarlo

    Salvedad: El programa solo funciona si se le ingresan números enteros positivos y no mayores a 60, ya que si no podría haber un desbordamiento de datos por culpa del que el número superaria la capacidad del unsigned long int. De lo contrario no se garantizan resultados 
...
// Inicio función PrintNarayanaSeries. Esta función va a imprimir la serie de Narayana hasta el número de terminos indicado y no devuelve nada
void PrintNarayanaSeries(unsigned long int n1, unsigned long int n2, unsigned long int n3, int limit) {
    // n1: Primer digito y digito imprimible de la serie de Narayana
    ...
    // Imprimimos la serie de Narayana siempre y cuando el limite no llegue a 0
    if ( limit > 0 ) { // Condición cota de la función para el llamado recursivo
        printf("%lu", n1); // Imprimimos el número de la serie de Narayana correspondiente, guardado en n1 
        ...
        PrintNarayanaSeries(n2, n3, n1 + n3, limit - 1); // Llamado recursivo a la función 
        // Pasa n2 como n1, n3 como n2, n1 + n3 como n3 y el limite se reduce en 1 por llamado para ir descontanto cada digito impreso
    } // Fin de la condición cota

    return; // No devuelve nada y finaliza la función
}// Fin función PrintNarayanaSeries
...
int main() { // Inicio función principal
    ...
    // Pedimos al usuario que ingrese el número de términos que desea ver de la serie de Narayana
    printf("\nIngrese el n%cmero de t%crminos que desea ver de la serie de Narayana: ", 163, 130);
    scanf("%i", &limit); // Se recibe el dato y se guarda dentro de la variable limit

    // Llamado a la función que va a imprimir la serie con los tres primeros digitos de esta (1, 1, 1) y el límite ingresado
    PrintNarayanaSeries(1, 1, 1, limit); 
