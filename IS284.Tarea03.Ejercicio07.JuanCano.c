...
- Descripción del programa: Este programa recibe por pantalla el número de términos e imprime de la serie de Narayana hasta ese número sin superarlo
    ...
    Salvedad: El programa solo funciona si se le ingresan números enteros positivos y no mayores a 60, ya que si no podría haber un desbordamiento de datos por culpa del que el número superaría la capacidad del unsigned long int. De lo contrario no se garantizan resultados 
...
// Inicio función PrintNarayanaSeries. Esta función va a imprimir la serie de Narayana hasta el número de términos indicado y no devuelve nada
void PrintNarayanaSeries(unsigned long int n1, unsigned long int n2, unsigned long int n3, int limit) {
    // n1: Primer dígito y dígito imprimible de la serie de Narayana
    ...
    // Imprimimos la serie de Narayana siempre y cuando el límite no llegue a 0
    if ( limit > 0 ) { // Condición cota de la función para el llamado recursivo
        ...
        // Pasa n2 como n1, n3 como n2, n1 + n3 como n3 y el límite se reduce en 1 por llamado para ir descontando cada dígito impreso
...
int main() { // Inicio función principal
    ...
    printf("\n");
    // Llamado a la función que va a imprimir la serie con los tres primeros dígitos de esta (1, 1, 1) y el límite ingresado
